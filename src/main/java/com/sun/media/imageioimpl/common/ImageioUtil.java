...
public final class ImageioUtil {
    ...
    // Hack to get the default imageio registry. Otherwise the method getDefaultInstance() is not thread safe (that will duplicate the registry).
    ...
    public static void registerServiceProvider(Class<?> clazz) {
        Class<?> spiClass = null;
        try {
            // If JRE contains imageio.jar in lib/ext, get SPI classes and unregister them (avoid to duplicate the services)
